...
Exp_data = np.loadtxt("data/U233_8MW_step.csv", delimiter = ",",skiprows = 1)
Exp_data = Exp_data.T 
plt.plot(Exp_data[0], Exp_data[1], color = "black", label = "ORNL-data")

data = np.loadtxt("data/flow.csv", skiprows=1,delimiter=",")
data = data.T
power = data[-4]*8/data[-4][0]
Delta_power = power-8
plt.plot(data[-1], Delta_power, color= "orange", label = "Squirrel flow")

data = np.loadtxt("data/solid_14pcm.csv", skiprows=1,delimiter=",")
data = data.T
power = data[-4]*8/data[-4][0]
Delta_power = power-8
plt.plot(data[-1], Delta_power, "--", color= "orange",label = "Squirrel static")

one_region_data = np.loadtxt("data/1_region_14.csv", delimiter = ",",skiprows = 1)
...
only_core_data = np.loadtxt("data/only_core_14.csv", delimiter = ",",skiprows = 1)
